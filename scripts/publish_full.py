"""完整发布流程 v2：真实点击上传视频 + 真实点击填文案（修复 React focus 问题）
用法: python publish_full.py <video> <title> <body> <tags>
"""
import json, time, sys, re, asyncio, os
...
async def main(video, title, body, tags, cdp_ws):
    ...
    async with websockets.connect(cdp_ws, max_size=2**30) as ws:
        ...
        # 优先 composer 页，否则取最后一个 business 页
        for t in meta_pages:
        ...
        if pos_s:
            ...
            print("⚠️ 无添加视频按钮（可能已有视频）")

        # 2. 填文案（真实点击输入框 + insertText）
        ...
        if not pos_s:
            print("❌ 找不到文案输入框"); return False
        pos = json.loads(pos_s)
        print(f"点击文案框 ({pos['x']},{pos['y']})")
        await real_click(pos["x"], pos["y"])
        ...
        if ver2 < 100:
            print("❌ 文案 3 秒后丢失，React 未接收"); return False
