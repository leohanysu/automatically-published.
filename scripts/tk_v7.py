"""TikTok 发布 v7（最终版）：上传视频 -> 等编辑页 -> 慢速标题 -> 逐个标签(等弹窗+回车) -> 验证变粗 -> Post
不点 Discard（用户明确：草稿不用管），只处理 Got it / Not now 引导
"""
...
async def main(cdp_ws, video, title, tags):
    ...
    async with websockets.connect(cdp_ws, max_size=2**30, open_timeout=15) as ws:
        ...
        # 2. 处理引导弹窗（Got it / Not now，不碰 Discard）
        ...
        # 3. 上传视频（Select video -> chooser -> setFileInputFiles）
        ...
        if not pos_s:
            print("❌ 无 Select video 按钮（可能已有视频）")
        else:
            ...
            print("✅ 视频已设置，等上传处理...")
        # 4. 等编辑页（Post 按钮 + Description 编辑器）
        for i in range(40):
