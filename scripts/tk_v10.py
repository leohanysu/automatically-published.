"""TikTok 发布 v10（最终）：上传(DOM直连input) -> 标题(真实键盘) -> 标签(弹窗出现后1.5s回车,不加空格) -> 验证 -> Post
2026-08-20 定稿参数：弹窗出现后等1.5秒回车；标签前不加空格；真实键盘逐字符
"""
...
async def main(cdp_ws, video, title, tags):
    ...
    async with websockets.connect(cdp_ws, max_size=2**30, open_timeout=15) as ws:
        ...
        # 2. 处理引导弹窗（Not now / Got it / Continue，不碰 Discard）
        ...
        # 3. 上传：DOM 直连 input[type=file]（带 sessionId）
        await asyncio.sleep(1)
        has_input = await ev("!!document.querySelector('input[type=file]')")
        print(f"input[type=file]: {has_input}")
        if has_input:
        ...
        for i, tag in enumerate(tags):
            ...
            for wait in range(16):
                ...
                if tag_clean in popup:
                    ...
                    break
            if appeared:
                await asyncio.sleep(1.5)  # 定稿：弹窗出现后等 1.5 秒再回车
            else:
        ...
        ver = await ev('''(() => {
            ...
            return JSON.stringify({len: t.length, head: t.slice(0,30), tail: t.slice(-80), links: links});
        })()''')
        v = json.loads(ver)
