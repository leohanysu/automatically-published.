"""Meta 发布 v2（NO_PROXY 修复代理拦截）：上传 -> 真实点击填文案 -> 下一页 -> 立即分享 -> 分享
端口从 DevToolsActivePort 动态读取
"""
import json, time, sys, re, asyncio, os, base64
...
async def main(video, title, body, tags, cdp_ws):
    ...
    async with websockets.connect(cdp_ws, max_size=2**30) as ws:
        ...
        # 2. 填文案（真实点击输入框 + insertText，验证过 React 100% 接收）
