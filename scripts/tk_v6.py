"""TikTok 全流程 v6：Continue草稿(不碰Discard) -> 编辑页 -> 清空 -> 慢速标题 -> 逐个标签(等弹窗+回车) -> 验证变粗 -> Post
2026-08-20 用户明确：不要点 Discard（删除要二次确认），点 Continue 继续编辑，草稿箱不用管
"""
...
async def main(cdp_ws, title, tags):
    ...
    async with websockets.connect(cdp_ws, max_size=2**30, open_timeout=15) as ws:
        ...
        # 1. 关弹窗：Not now / Got it / Continue（禁止 Discard！）
        for attempt in range(6):
            r = await ev('''(() => {
                const btns = [...document.querySelectorAll('button')].filter(b => b.offsetWidth > 0);
                const txt = (b) => (b.innerText||'').trim();
                const continue_ = btns.find(b => txt(b) === 'Continue' && b.getBoundingClientRect().width > 50);
                if (continue_) { continue_.click(); return 'CONTINUE'; }
                const notNow = btns.find(b => txt(b) === 'Not now');
                if (notNow) { notNow.click(); return 'NOT_NOW'; }
                const gotIt = btns.find(b => txt(b) === 'Got it');
        ...
        for i in range(15):
            ...
            if has_editor:
                print(f"✅ 编辑页就绪 ({i*2}s)")
                break
        # 3. 点击编辑器 + 清空（Ctrl+A + Backspace x2）
        pos_s = await ev('''(() => {
        ...
        open(r"C:\Users\Administrator\Downloads\feishu_videos\tiktok_v6.png", "wb").write(base64.b64decode(b64))
        print("截图 tiktok_v6.png")
        return True
